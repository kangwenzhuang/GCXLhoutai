<template>
	<section>
		...
		<el-table :data="users" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="classes" label="课程" width="130" sortable>
			</el-table-column>
		...
		<el-dialog title="编辑" v-model="editFormVisible" :close-on-click-modal="false">
			<el-form :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
				...
				<el-form-item label="课程" prop="classes">
					<el-input type="textarea" v-model="editForm.classes"></el-input>
				</el-form-item>
	...
	export default {
		...
		methods: {
			...
			async userList() {
				...
				await this.$http
						.get("http://kangkangin.asia:8080/signin/users?pageNum="+this.page+"&pageSize="+this.pagesize+"&shenfen=0")
						.then(res => {
							...
							console.log(this.users[0].id);
							 this.$http.post("http://kangkangin.asia:8080/signin/getCreateCourse",{
							 	'userid':this.users[0].id
							 }).then(res=>{
							 	this.courses=res.data.data.getCreateCourse;

								console.log("what is this============="+this.courses[0].kcm);
							})
						});
			},
			//查询用户
			async findUser(){
				...
				if(para.name=='') this.userList();
				else {
					await this.$http.get("http://kangkangin.asia:8080/signin/users?name=" + para.name + "&shenfen=0")
							.then(res => {
								this.listLoading = false;
								this.users = res.data.data.userList.list;
							})
			...
			handleDel: function (index, row) {
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					await this.$http.delete("http://kangkangin.asia:8080/signin/users?xuehao=" + para.xuehao + "&shenfen=0")
							.then(res=>{
								this.userList();
							})
				}).catch(() => {
				...
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							this.$http.put("http://kangkangin.asia:8080/signin/users?xuehao="+para.xuehao+"&shenfen=0", {
